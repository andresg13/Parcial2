...
def main() -> None:
    ...
    factories_types = [2, 1, 2, 1, 2, 2, 2, 2,
                       2, 2, 1, 1, 2, 1, 1, 1, 2, 2, 1, 1, 2]
    model_numbers = [5, 5, 4, 3, 1, 2, 4, 4,
                     4, 2, 5, 3, 4, 2, 4, 2, 3, 5, 3, 2, 2]
    consumptions = ['Very High', 'Low', 'Very Low', 'Very High', 'High', 'Medium', 'High', 'Low', 'Medium', 'High',
                    'Very High', 'Medium', 'Medium', 'Very High', 'Very High', 'Medium', 'Medium', 'Low', 'High', 'Very Low', 'Medium']
    weights = [0.07, 0.21, 1.55, 1.72, 0.35, 0.52, 1.45, 1.78, 0.34, 1.79,
               0.49, 0.09, 0.74, 0.87, 1.28, 0.35, 1.08, 0.57, 1.32, 1.89, 1.80]
    costs = [0.13, 43.81, 17.65, 10.16, 2.99, 31.09, 38.97, 16.03, 47.35, 27.31,
             35.89, 30.81, 14.33, 13.04, 45.88, 7.74, 44.64, 49.63, 18.93, 39.39, 33.08]

    for (factory_type, model_number, consumption, weight, cost) in zip(factories_types, model_numbers, consumptions, weights, costs):
        service.add_client(
            Client(factory_type, model_number, consumption, weight, cost))

    service.factories_summary()

    client_id, food_type, food_cost = service.client_most_expensive_food()
    print(
        f'Client\'s {client_id} {food_type} is the most expensive with {food_cost} dollars')
